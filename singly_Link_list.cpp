...
List* createList(NODE* p_node)
{
    ...
    NODE* temp = p_node;
    while (temp && temp->p_next != nullptr) {
        temp = temp->p_next;
    }
    newList->p_tail = temp;
...
bool addBefore(List*& L, int data, int val)
{
	...
	NODE* current = L->p_head;
    while (current && current->p_next != nullptr) {
        if (current->p_next->key == val) {
            newNode->p_next = current->p_next;
			current->p_next = newNode;
            return true;
...
bool addAfter(List*& L, int data, int val)
{
    ...
    NODE* current = L->p_head;
        ...
        current = current->p_next;
        if (current == L->p_tail && current->key==L->p_tail->key) {
			L->p_tail = newNode;
        }
    }
    return false;
...
List* reverseList(List* L)
{
    ...
    while (current != nullptr) {
        ...
        newNode->p_next = reList->p_head;
        reList->p_head = newNode;
