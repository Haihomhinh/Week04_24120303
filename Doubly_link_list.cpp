	...
	while (current != nullptr) {
		if (current->key == val) {
			...
			if (toDelete == L->p_head) {
				...
				toDelete->p_prev->p_next = current;
				current->p_prev = toDelete->p_prev;
			}
			delete toDelete;
			return;
		}
...
void removeAfter(List*& L, int val)
{
    ...
    while (current != nullptr) {
        ...
        if(current->key==val){
            NODE* toDelete = current->p_next;

			if (toDelete == nullptr) {
				return;
			}
            current->p_next = toDelete->p_next;
            
			if (toDelete->p_next != nullptr) {
				toDelete->p_next->p_prev = current;
			}
			else {
				L->p_tail = current;
			}
...
void printList(List* L)
{
    // Your code here //
    List* temp = L;
    while (temp->p_head != nullptr) {
		cout << temp->p_head->key << " ";
		temp->p_head = temp->p_head->p_next;
    }
    return;
}
...
List* reverseList(List* L)
{
    ...
    while (current != nullptr) {
        temp = current->p_prev;
        current->p_prev = current->p_next;
        current->p_next = temp;
		current = current->p_prev;
    }
    if (current != nullptr) {
        L -> p_head = temp->p_prev;
    }
    return L;
}
